...
import net.minecraft.src.GuiMinetapMenu;
...
import me.sempre234.ocipe.Ocipe;

public abstract class Minecraft implements Runnable, IPlayerUsage {
	
	public static byte[] memoryReserve = new byte[10485760];
	private final ILogAgent field_94139_O = new LogAgent("Minecraft-Client", " [CLIENT]",
			(new File(getMinecraftDir(), "output-client.log")).getAbsolutePath());
	...
	public void startGame() throws LWJGLException {
		...
		this.standardGalacticFontRenderer = new FontRenderer(this.gameSettings, "/font/alternate.png",
	...
	private void loadScreen() throws LWJGLException {
		...
		short var3 = 256;
		short var4 = 256;
		this.scaledTessellator((var1.getScaledWidth() - var3) / 2, (var1.getScaledHeight() - var4) / 2, 0, 0, var3,
				var4);
		GL11.glDisable(GL11.GL_LIGHTING);
		GL11.glDisable(GL11.GL_FOG);
		GL11.glEnable(GL11.GL_ALPHA_TEST);
	...
	public void scaledTessellator(int par1, int par2, int par3, int par4, int par5, int par6) {
		...
		var9.addVertexWithUV((double) (par1 + 0), (double) (par2 + par6), 0.0D, (double) ((float) (par3 + 0) * var7),
				(double) ((float) (par4 + par6) * var8));
		var9.addVertexWithUV((double) (par1 + par5), (double) (par2 + par6), 0.0D,
				(double) ((float) (par3 + par5) * var7), (double) ((float) (par4 + par6) * var8));
		var9.addVertexWithUV((double) (par1 + par5), (double) (par2 + 0), 0.0D, (double) ((float) (par3 + par5) * var7),
				(double) ((float) (par4 + 0) * var8));
		var9.addVertexWithUV((double) (par1 + 0), (double) (par2 + 0), 0.0D, (double) ((float) (par3 + 0) * var7),
				(double) ((float) (par4 + 0) * var8));
		var9.draw();
	}
						...
						: (var0.contains("solaris") ? EnumOS.SOLARIS
								: (var0.contains("sunos") ? EnumOS.SOLARIS
										: (var0.contains("linux") ? EnumOS.LINUX
												: (var0.contains("unix") ? EnumOS.LINUX : EnumOS.UNKNOWN)))));
	}
	...
	public void run() {
		...
		try {
			...
			this.displayCrashReport(
					this.addGraphicsAndWorldToCrashReport(new CrashReport("Failed to start game", var11)));
			return;
		}
	...
	private void runGameLoop() {
		if (this.mcApplet != null && !this.mcApplet.isActive()) {
			...
			if (this.mcCanvas != null && !this.fullscreen && (this.mcCanvas.getWidth() != this.displayWidth
					|| this.mcCanvas.getHeight() != this.displayHeight)) {
			...
			this.isGamePaused = this.isSingleplayer() && this.currentScreen != null
					&& this.currentScreen.doesGuiPauseGame() && !this.theIntegratedServer.getPublic();

			if (this.isIntegratedServerRunning() && this.thePlayer != null && this.thePlayer.sendQueue != null
					&& this.isGamePaused != var5) {
	...
	private int func_90020_K() {
		return this.currentScreen != null && this.currentScreen instanceof GuiMainMenu ? 2
		...
		if (Keyboard.isKeyDown(60)) {
			if (!this.isTakingScreenshot) {
				...
				this.ingameGUI.getChatGUI().printChatMessage(
						ScreenShotHelper.saveScreenshot(minecraftDir, this.displayWidth, this.displayHeight));
			}
		} else {
	...
	private void displayDebugInfo(long par1) {
		if (this.mcProfiler.profilingEnabled) {
			...
			this.fontRenderer.drawStringWithShadow(var18 = var19.format(var4.field_76330_b) + "%",
					var7 + var6 - this.fontRenderer.getStringWidth(var18), var8 - var6 / 2 - 16, var13);
			...
			for (int var21 = 0; var21 < var3.size(); ++var21) {
				...
				this.fontRenderer.drawStringWithShadow(var22, var7 - var6, var8 + var6 / 2 + var21 * 8 + 20,
						var20.func_76329_a());
				this.fontRenderer.drawStringWithShadow(var22 = var19.format(var20.field_76332_a) + "%",
						var7 + var6 - 50 - this.fontRenderer.getStringWidth(var22), var8 + var6 / 2 + var21 * 8 + 20,
						var20.func_76329_a());
				this.fontRenderer.drawStringWithShadow(var22 = var19.format(var20.field_76330_b) + "%",
						var7 + var6 - this.fontRenderer.getStringWidth(var22), var8 + var6 / 2 + var21 * 8 + 20,
						var20.func_76329_a());
			}
		}
	}
	...
	private void sendClickBlockToController(int par1, boolean par2) {
		...
		if (par1 != 0 || this.leftClickCounter <= 0) {
			if (par2 && this.objectMouseOver != null && this.objectMouseOver.typeOfHit == EnumMovingObjectType.TILE
					&& par1 == 0) {
		...
		if (par1 != 0 || this.leftClickCounter <= 0) {
			...
			if (this.objectMouseOver == null) {
				...
				if (par1 == 0) {
					...
					if (this.playerController.onPlayerRightClick(this.thePlayer, this.theWorld, var3, var4, var5, var6,
							var7, this.objectMouseOver.hitVec)) {
						var2 = false;
						this.thePlayer.swingItem();
					}
	...
	public void runTick() {
		...
		if (!this.isGamePaused && this.thePlayer != null) {
			Ocipe.moduleManager.onUpdate();
		}
		...
		if (this.currentScreen == null || this.currentScreen.allowUserInput) {
			...
			while (Mouse.next()) {
				...
				if (Mouse.getEventButtonState() && Mouse.getEventButton() == 0 && this.currentScreen == null) {
					Ocipe.moduleManager.recordClick();
				}
				...
				if (Keyboard.getEventKeyState()) {
					if (Keyboard.getEventKey() == 87) {
						...
						if (this.currentScreen != null) {
							...
							if (Keyboard.getEventKey() == Keyboard.KEY_INSERT) {
								this.displayGuiScreen(new GuiMinetapMenu());
								continue;
							}
							
							Ocipe.moduleManager.onKey(Keyboard.getEventKey());
			...
			if (this.gameSettings.keyBindUseItem.pressed && this.rightClickDelayTimer == 0
					&& !this.thePlayer.isUsingItem()) {
				this.clickMouse(1);
			}

			this.sendClickBlockToController(0,
					this.currentScreen == null && this.gameSettings.keyBindAttack.pressed && this.inGameHasFocus);
		}
	...
	public void launchIntegratedServer(String par1Str, String par2Str, WorldSettings par3WorldSettings) {
		...
		try {
			...
			this.displayCrashReport(
					this.addGraphicsAndWorldToCrashReport(new CrashReport("Connecting to integrated server", var8)));
		}
	}
	...
	public void addServerStatsToSnooper(PlayerUsageSnooper par1PlayerUsageSnooper) {
		...
		par1PlayerUsageSnooper.addData("texpack_name",
				this.texturePackList.getSelectedTexturePack().getTexturePackFileName());
	...
	public void addServerTypeToSnooper(PlayerUsageSnooper par1PlayerUsageSnooper) {
		...
		par1PlayerUsageSnooper.addData("gl_caps[gl_max_fragment_uniforms]",
