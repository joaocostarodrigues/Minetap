...
public class EntityClientPlayerMP extends EntityPlayerSP
{
    ...
    public void sendMotionUpdates()
    {
        ...
        boolean reportedOnGround = FallDamege.enabled || this.onGround;

        if (this.ridingEntity != null)
        {
            this.sendQueue.addToSendQueue(new Packet13PlayerLookMove(this.motionX, -999.0D, -999.0D, this.motionZ, this.rotationYaw, this.rotationPitch, reportedOnGround));
            var13 = false;
        }
        else if (var13 && var14)
        {
            this.sendQueue.addToSendQueue(new Packet13PlayerLookMove(this.posX, this.boundingBox.minY, this.posY, this.posZ, this.rotationYaw, this.rotationPitch, reportedOnGround));
        }
        else if (var13)
        {
            this.sendQueue.addToSendQueue(new Packet11PlayerPosition(this.posX, this.boundingBox.minY, this.posY, this.posZ, reportedOnGround));
        }
        else if (var14)
        {
            this.sendQueue.addToSendQueue(new Packet12PlayerLook(this.rotationYaw, this.rotationPitch, reportedOnGround));
        }
        else
        {
            this.sendQueue.addToSendQueue(new Packet10Flying(reportedOnGround));
        }

        ++this.field_71168_co;
        this.wasOnGround = reportedOnGround;
